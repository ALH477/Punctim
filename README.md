...
## Overview
Punctim is a free and open-source software (FOSS) framework evolved from the DeMoD Secure Protocol (a historical name only: DCF has no confidentiality, authentication or tamper protection — see [`Documentation/DCF_SECURITY_EXPOSURE.md`](Documentation/DCF_SECURITY_EXPOSURE.md)), designed for low-latency, modular, and interoperable data exchange. It targets applications like IoT messaging, real-time gaming synchronization, distributed computing, and edge networking. Punctim features a handshakeless design and a compatibility layer for UDP, TCP, WebSocket, and gRPC transports, aiming at peer-to-peer (P2P) networking with self-healing redundancy.
...
| **Certified — needs an external compiler** | **Exsecutor** ([`exsecutor/`](exsecutor/), codec relicensed `LGPL-3.0-only`; compiler [`ALH477/exsecutor`](https://github.com/ALH477/exsecutor) stays `GPL-3.0-or-later`) | `exsecutor/certify.sh` compiles the in-tree `DeModFrame` codec and checks it against this repo's live `Documentation/golden_vectors.json`: **246/246 vectors** (109 encode + 137 syndrome), 3/3 CRC anchors, 218/218 + 136/136 laws, and a determinism check. Unlike the rows above it is **not self-contained** — certifying it needs the `exsc` compiler, which this repo does not ship and builds on demand (`nix build .#exsc`); without that toolchain `certify.sh` skips rather than fails. Upstream additionally runs mutation checks — a deliberately broken CRC polynomial, byte order or nibble order must fail at a named vector — which no other binding here does. See [`Documentation/DCF_EXSECUTOR.md`](Documentation/DCF_EXSECUTOR.md) and [`LICENSING.md`](LICENSING.md). | — |
...
kind: file_contains
severity: error
path: hydramodem/README.md
pattern: opaqu
-->
`hydramodem/README.md` states the transport carries the 17-byte `DeModFrame` opaquely — HydraModem never parses the frame.
<!-- truth:end -->
<!-- truth:claim
id: medium-cert-symbols
kind: file_exists
severity: error
path: Documentation/medium_vectors.json
-->
`Documentation/medium_vectors.json` exists; per `DCF_MEDIUM_SPEC.md`, DCF-Medium certification stops at the symbol/bit stream for analog media (HydraModem's waveform is loopback-tested, not byte-certified).
<!-- truth:end -->
...
- **HydraModem (acoustic M-FSK PHY, `hydramodem/`)**: a self-contained LGPL-3.0 C library (relicensed from Apache-2.0 on integration) that carries the 17-byte frame over **sound** with a real receiver — continuous-phase **M-FSK**, preamble/sync acquisition, **symbol-timing recovery (±3000 ppm)**, soft-Viterbi conv FEC + interleaver, and a streaming RX. A transport *beneath* the quantum (carries the frame opaquely, wire certificate untouched; CRC anchor `0x29B1`). Its **physical layer is authored in Faust** — the CPFSK modulator and quadrature demod bank are the normative `.dsp` — with a **C reference DSP** (a 1:1 port of the `.dsp`) as the default build and a **compiled-Faust backend** (`nix build .#hydramodem-faust`; version-tolerant across **Faust 2.72–2.85**) verified decode-equivalent (the waveforms differ only in carrier phase): cross-decoded both ways and matched over the cable. Its default 1000-baud profile is a near-field/cabled link and its timing recovery handles two interfaces' independent sample clocks — proven over real hardware (two cross-cabled USB interfaces: PER 0% sequential over 200 frames each way; in full duplex, zero cross-link bleed in both directions, but PER 0% only A→B — B→A was ~4% (287/300) — via `hydramodem/dcf-tools/`; reported in `hydramodem/docs/WHITEPAPER.md` §4, raw logs not committed). Its certificate covers the symbol stream only; which profile is tested, certified, independently re-implemented or field-validated is tabulated in [`hydramodem/README.md`](hydramodem/README.md#what-is-established-and-at-which-level), with a glossary of the project's names. `nix build .#hydramodem`.
